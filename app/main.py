...
# Crear cliente para aceder a rdis 
...
def index():
    ...
    """
   # contador_visitas = contador_visitas + 1  / incr operacion atomica

    contador_visitas = redis_client.incr((CONTADOR_KEY));
 #Crea condicion de carrera si acceden varios equipos   
 #  if not contador_visitas:
  ##     contador_visitas = 1 
 #  else:
#       contador_visitas = contador_visitas + 1
#       redis_cli.set(CONTADOR_KEY, contador_vistas)


    return flask.render_template(
            'index.html', nombre=nombre,contador_visitas=contador_visitas,
